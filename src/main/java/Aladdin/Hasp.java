...
public class Hasp {
	...
	public static final String HASP_RECIPIENT = new String(
			"<haspformat root=\"location\">" + "    <license_manager>" + "    <attribute name=\"id\" />" + "    <attribute name=\"time\" />" + "    <element name=\"hostname\" />"
					+ "    <element name=\"version\" />" + "    <element name=\"host_fingerprint\" />" + "  </license_manager>" + "</haspformat> \n");
	...
	/**
	 * After AND-ing with HASP_FEATURETYPE_MASK, the Feature type contains this
	 * value.
	 */
	public static final long HASP_PROGNUM_FEATURETYPE = 0xffff0000;

	/**
	 * AND-mask used to extract program number from Feature ID if program number
	 * Feature.
	 */
	...
	/**
	 * "Prognum" option: enables the API to access "classic" (HASP4 or earlier)
	 * keys.
	 */
	...
	/**
	 * Minimal block size for legacy functions hasp_legacy_encrypt() and
	 * hasp_legacy_decrypt().
	 */
	...
	/**
	 * HASP4 FAS memory file: (Dummy) File ID for license data segment of memory
	 * contents.
	 */
	 ...
	 * <p>
	 * Network "prognum" Features only use the old HASP LM login logic, with all its
	 * limitations.
	 * <p>
	 ...
	 *            Unique identifier of the Feature.
	 * 
	 *            With "prognum" Features (see HASP_FEATURETYPE_MASK), 8 bits are
	 *            reserved for legacy options (see HASP_PROGNUM_OPT_MASK, currently
	 *            5 bits are used):
	 *            <ul>
	 *            <li>only local
	 *            <li>only remote
	 ...
	 * 
	 * This function is an extended login function, where the search for the Feature
	 * can be restricted.
	 * <p>
	 * If a previously established session context exists, the session will be
	 * logged out.
	 * 
	 * @param scope
	 *            The hasp_scope of the Feature search.
	 * @param vendor_code
	 *            The Vendor Code.
	 * 
	 * @return true/false - indicates success or failure.
	 ...
	 * <p>
	 * If the encryption fails (e.g. key removed during the process) the data buffer
	 * is unmodified.
	 * <p>
	 ...
	 * @param length
	 *            Size in bytes of the buffer to be encrypted (16 bytes minimum).
	 * 
	 * @return true/false - indicates success or failure.
	 * 
	 * @see #decrypt
	 ...
	 * <p>
	 * If the decryption fails (e.g. key removed during the process) the data buffer
	 * is unmodified.
	 * <p>
	 * This function is deprecated.
	 * 
	 ...
	 * @param length
	 *            Size in bytes of the buffer to be decrypted (16 bytes minimum).
	 * 
	 * @return true/false - indicates success or failure.
	 * 
	 * @see #encrypt
	 ...
	 * 
	 * Acquires information about all system components. The programmer can choose
	 * the scope and output structure of the data. The function has a "scope"
	 * parameter that defines the scope using XML syntax.
	 * <p>
	 ...
	 * 
	 * @param fileid
	 *            ID of the file to read (memory descriptor).
	 * @param offset
	 ...
	 * 
	 * Depending on the provided session handle (either logged into the default
	 * Feature or any other Feature), write access to the FAS memory
	 * (HASP_FILEID_LICENSE) is not permitted.
	 * <p>
	 * This function is deprecated.
	 * 
	 * @param fileid
	 *            ID of the file to write (memory descriptor).
	 * @param offset
	 *            Position in the file.
	 * @param length
	 ...
	 * 
	 * The update BLOB contains all necessary data to perform the update: Where to
	 * write (to which HASP key), the necessary access data (Vendor Code) and the
	 * update itself.
	 * <p>
	 * If requested by the update BLOB, the function returns an Acknowledge BLOB,
	 * which is signed/encrypted by the updated instance and contains proof that
	 * this update was successfully installed.
	 * 
	 * @param update_data
	 ...
	 * 
	 * Time values are returned as the number of seconds that have elapsed since
	 * Jan-01-1970 0:00:00 UTC.
	 * <p>
	 * The general purpose of this function is to obtain reliable timestamps that
	 * are independent from the system clock.
	 * 
	 ...
	 * 
	 * @param idle_time
	 *            Idle time in minutes. Set to 0 for default value.
	 * 
	 * @return true/false - indicates success or failure.
	 * 
	 * @see #getLastError
	 */
	/*
	 * public boolean setIdletime(short idle_time) { status =
	 * Hasp.SetIdletime(handle[0], idle_time); if (status ==
	 * HaspStatus.HASP_STATUS_OK) return true;
	 * 
	 * return false; }
	 */
	 ...
	 * 
	 * @param buffer
	 *            The buffer to be encrypted. Minimum size: 16 bytes
	 * 
	 * @return true/false - indicates success or failure.
	 * 
	 * @see #decrypt
	 ...
	 * 
	 * @param buffer
	 *            The buffer to be decrypted. Minimum size: 16 bytes
	 * 
	 * @return true/false - indicates success or failure.
	 * 
	 * @see #encrypt
	 ...
	 * @param action
	 *            XML definition of the action.
	 * @param scope
	 *            XML definition of the information scope.
	 * @param vendor_code
	 *            The Vendor Code.
	 * @param destination
	 *            XML definition of the output destination.
	 * @return info The returned information (XML list).
	 * 
	 * @see #getLastError
	 */
