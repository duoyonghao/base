...
@Service
public class SysDeptPrivServiceImpl
  implements ISysDeptPrivService
{
  @Autowired
  ...
  @Transactional(rollbackFor={Exception.class})
  public void insertSysDeptPriv(Map<String, String> map)
    throws Exception
  {
    SysDeptPriv sysDeptPriv = new SysDeptPriv();
    sysDeptPriv.setSeqId(YZUtility.getUUID());
    sysDeptPriv.setButtonName((String)map.get("buttonname"));
    sysDeptPriv.setCreatetime(YZUtility.getCurDateTimeStr("yyyy-MM-dd HH:mm:ss"));
    sysDeptPriv.setCreateuser((String)map.get("createuser"));
    sysDeptPriv.setDeptNoCompilations((String)map.get("deptNoCompilations"));
    sysDeptPriv.setDeptName((String)map.get("deptName"));
    sysDeptPriv.setDeptType((String)map.get("deptType"));
    sysDeptPriv.setOrganization((String)map.get("organization"));
    this.sysDeptPrivDao.insertSysDeptPriv(sysDeptPriv);
  }
  
  @Transactional(rollbackFor={Exception.class})
  public void deleteSysDeptPriv(String deptprivid)
    throws Exception
  {
  ...
  @Transactional(rollbackFor={Exception.class})
  public int updateSysDeptPriv(Map<String, String> map)
    throws Exception
  {
    SysDeptPriv sysDeptPriv = new SysDeptPriv();
    sysDeptPriv.setSeqId((String)map.get("deptprivid"));
    sysDeptPriv.setUpdatetime(YZUtility.getCurDateTimeStr("yyyy-MM-dd HH:mm:ss"));
    sysDeptPriv.setUpdateuser((String)map.get("createuser"));
    sysDeptPriv.setDeptNoCompilations((String)map.get("deptNoCompilations"));
    sysDeptPriv.setDeptName((String)map.get("deptName"));
    sysDeptPriv.setDeptType((String)map.get("deptType"));
    ...
    throws Exception
  {
    ...
    {
      if (!YZUtility.isNullorEmpty((String)map.get("buttonname")))
      {
        List<JSONObject> list = this.sysDeptPrivDao.findDeptNameByButtonName(map);
        if (list.size() > 0)
        {
    ...
    else if (!YZUtility.isNullorEmpty((String)map.get("buttonname")))
    {
