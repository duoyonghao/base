...
@Service
public class HUDH_DrugsFileUploadLogic
  extends BaseLogic
{
  ...
  public JSONObject fileUploadAndImport(String module, YZFileUploadForm fileForm, HttpServletRequest request, String imgtype, String organization)
    throws Exception
  {
    ...
    try
    {
      ...
      while (iKeys.hasNext())
      {
        String fieldName = (String)iKeys.next();
  ...
  private String exceltodSavedata(HttpServletRequest request, FileItem fileitem, String mtype, String organization)
    throws Exception
  {
    ...
    for (int i = 1; i < list.size(); i++)
    {
      ...
      List<String> rows = (List)list.get(i);
      String value = "";
      for (int j = 0; j < rowslength; j++)
      {
        String li = (String)rows.get(j);
  ...
  private void importDrugs(String[] val, YZPerson person, HttpServletRequest request, String organization)
    throws Exception
  {
    ...
    ykzzDrugs.setGood_name(val[0]);
    BigDecimal bd = new BigDecimal(val[1]);
    bd = bd.setScale(3, 4);
    ykzzDrugs.setDrug_retail_price(bd);
    ykzzDrugs.setDiscount(val[2]);
    ykzzDrugs.setChemistry_name(val[3]);
    ykzzDrugs.setPharm_spec(val[4]);
    ykzzDrugs.setCompany(val[5]);
    ykzzDrugs.setDrugs_typeone(val[6]);
    ykzzDrugs.setDrugs_typetwo(val[7]);
    ykzzDrugs.setPacking_num(val[8]);
    ykzzDrugs.setCompany_min(val[9]);
    ykzzDrugs.setContent_coe(val[10]);
    ykzzDrugs.setContent_unit(val[11]);
    ykzzDrugs.setSta_item(val[12]);
    ykzzDrugs.setContry_code(val[13]);
    ykzzDrugs.setPharm_class(val[14]);
    ykzzDrugs.setPharm_dos(val[15]);
    ykzzDrugs.setAnt_gra(val[16]);
    ykzzDrugs.setPsy_drugs(val[17]);
    ykzzDrugs.setMinnums(val[18]);
    ykzzDrugs.setMingj(val[19]);
    ykzzDrugs.setMaxnums(val[20]);
    ykzzDrugs.setMingj(val[21]);
    ykzzDrugs.setDrug_identify(val[22]);
    ykzzDrugs.setComments(val[23]);
    ykzzDrugs.setManu_id(val[24]);
    ykzzDrugs.setPacking_unit(val[25]);
    this.IYkzzDrugsDao.insertDrugsInfro(ykzzDrugs);
    ...
    this.IYkzzDrugsDao.addTreatItemBack(kqdsTreatitem);
  }
  
  public JSONObject fileUploadLogicForbase64(String module, String imgStr, HttpServletRequest request)
    throws Exception
  {
    JSONObject result = new JSONObject();
    try
  ...
  public String importPaiBanData(HttpServletRequest request, FileItem fileitem, String mtype, String organization)
    throws Exception
  {
    YZPerson person = SessionUtil.getLoginPerson(request);
    ...
    List<String> dateRowList = (List)list.get(0);
    ...
    for (int i = 1; i < list.size(); i++)
    {
      List<String> rowDataList = (List)list.get(i);
      if (YZUtility.isNullorEmpty((String)rowDataList.get(0))) {
        break;
      }
      String userId = (String)rowDataList.get(0);
      if (YZUtility.isNullorEmpty(userId)) {
        throw new Exception("第" + (i + 1) + "行用户账号不能为空，请重新导入！");
      }
      userIdList.add(userId);
    }
    Object userId2SeqIdMap = new HashMap();
    Map<String, String> userId2DeptIdMap = new HashMap();
    List<YZPerson> perlist;
    for (int i = 0; i < userIdList.size(); i++)
    {
      String userId = (String)userIdList.get(i);
      Map<String, String> filtermap = new HashMap();
      filtermap.put("user_id", userId);
      perlist = (List)this.dao.loadList(TableNameUtil.SYS_PERSON, filtermap);
      if ((perlist == null) || (perlist.size() == 0)) {
        throw new Exception("第" + (i + 2) + "行用户账号在系统中不存在，请重新导入！");
      }
      ((Map)userId2SeqIdMap).put(userId, ((YZPerson)perlist.get(0)).getSeqId());
      userId2DeptIdMap.put(userId, ((YZPerson)perlist.get(0)).getDeptId());
    }
    List<KqdsPaibanType> plist = (List)this.dao.loadList(TableNameUtil.KQDS_PAIBAN_TYPE, null);
    ...
    for (int i = 1; i < list.size(); i++)
    {
      ...
      String userId = (String)rowDataList.get(0);
      for (int j = 1; j < dateRowList.size(); j++)
      {
        String zbValue = (String)rowDataList.get(j);
        ...
        KqdsPaibanType pbTypeObj = (KqdsPaibanType)pbTypeMap.get(zbValue);
        

        Map<String, String> pbfilter = new HashMap();
        pbfilter.put("personid", (String)((Map)userId2SeqIdMap).get(userId));
        pbfilter.put("startdate", (String)dateRowList.get(j));
        ...
        if (SeqId != null)
        {
          ...
          entity.setPersonid((String)((Map)userId2SeqIdMap).get(userId));
          entity.setDeptid((String)userId2DeptIdMap.get(userId));
          if (!YZUtility.isNullorEmpty(pbTypeObj.getStartdate()))
          {
            entity.setStartdate((String)dateRowList.get(j) + " " + pbTypeObj.getStartdate() + ":00");
            entity.setEnddate((String)dateRowList.get(j) + " " + pbTypeObj.getEnddate() + ":00");
          }
          else
          {
            entity.setStartdate((String)dateRowList.get(j) + ConstUtil.TIME_START);
            entity.setEnddate((String)dateRowList.get(j) + ConstUtil.TIME_START);
          }
  ...
  private String checkPaiban4DaoRu(Map<String, String> map, String organization)
    throws Exception
  {
    List<JSONObject> list = (List)this.dao.loadList(TableNameUtil.KQDS_PAIBAN, map);
    if ((list != null) && (list.size() > 0)) {
  ...
  public void importDeptUser(String[] val, YZPerson person, HttpServletRequest request)
    throws Exception
  {
    String userId = null;
  ...
  public void importCKGoods(String[] val, YZPerson person, HttpServletRequest request, String organization)
    throws Exception
  {
    if (YZUtility.isNullorEmpty(val[0])) {
    ...
    List<KqdsCkGoodsDetail> listDetail = (List)this.dao.loadList(TableNameUtil.KQDS_CK_GOODS_DETAIL, map2);
    ...
    List<KqdsCkGoodsType> lelve1List = (List)this.dao.loadList(TableNameUtil.KQDS_CK_GOODS_TYPE, map1);
    ...
    if (!YZUtility.isNullorEmpty(val[10]))
    {
      ...
      List<KqdsCkSupplier> supplierlist = (List)this.dao.loadList(TableNameUtil.KQDS_CK_SUPPLIER, map4);
  ...
  private KqdsCkGoodsType saveGoodsType(String goodstypename, String perid, YZPerson person, HttpServletRequest request, String organization)
    throws Exception
  {
  ...
  private KqdsCkHouse saveHouse(String housename, YZPerson person, HttpServletRequest request, String organization)
    throws Exception
  {
  ...
  private KqdsCkSupplier saveSupplier(String suppliername, String suppliercode, YZPerson person, HttpServletRequest request, String organization)
    throws Exception
  {
  ...
  public void importUserDocument(String[] val, YZPerson person, HttpServletRequest request, String organization)
    throws Exception
  {
    if (YZUtility.isNullorEmpty(val[0])) {
  ...
  public void importCostItems(String[] val, YZPerson person, HttpServletRequest request, String organization)
    throws Exception
  {
    if (val.length != 12) {
  ...
  private void importOutProcessingItem(String[] val, YZPerson person, HttpServletRequest request, String organization)
    throws Exception
  {
    if (YZUtility.isNullorEmpty(val[0])) {
      throw new Exception("项目编号不能为空");
  ...
  private void importInformation(String[] val, YZPerson person, HttpServletRequest request, String organization)
    throws Exception
  {
    String xxtype = val[0];
    ...
    for (int i = 0; i < xxcontentArr.length; i++)
    {
      ...
      if (!content.equals("")) {
        xxcontent = xxcontent + start_p + content + end_p;
      }
    }
  ...
  public void importDictData(String[] val, YZPerson person, HttpServletRequest request, String organization)
    throws Exception
  {
    int useflag2 = 0;
    int useflag3 = 0;
