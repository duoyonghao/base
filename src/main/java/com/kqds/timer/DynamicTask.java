...
public class DynamicTask
{
  ...
  public boolean startOrStop(YZTimeTask task, boolean start)
    throws Exception
  {
    ...
    if (isSuccess)
    {
      ...
      this.log.info((start ? "开启任务" : "停止任务") + "-------TaskId:" + task.getTaskId() + "-------Describe:" + task.getTaskDescribe() + "-----ClassName:" + task.getClassName());
    }
    return isSuccess;
  }
  
  public boolean updateCronExpression(YZTimeTask task)
    throws Exception
  {
    try
    {
      ...
      if ("1".equals(task.getIsStart()))
      {
        ...
        if (!originExpression.equalsIgnoreCase(newExpression))
        {
          ...
          this.schedulerFactory.rescheduleJob("cron_" + task.getSeqId(), "DEFAULT", trigger);
        }
      }
      else
      {
  ...
  public void loadTask()
  {
    ...
    try
    {
      tasks = (List)this.timeTaskService.loadList("SYS_TIMETASK", filterMap);
    }
    catch (Exception e)
    {
      e.printStackTrace();
