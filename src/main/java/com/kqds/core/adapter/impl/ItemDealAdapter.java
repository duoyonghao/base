...
public class ItemDealAdapter
  implements DealAdapter
{
  public boolean isDeal(KqdsReg reg, KqdsCostorder cost, List<KqdsCostorderDetail> list, HttpServletRequest request, Kqds_PayCostLogic logic, YZDictLogic dictLogic, KQDS_UserDocumentLogic userLogic)
    throws Exception
  {
    ...
    for (KqdsCostorderDetail detail : list) {
      if ((detail.getIstsxm() == null) || (detail.getIstsxm().intValue() == 0)) {
        tsxmflag = true;
      } else if ((detail.getIstsxm() == null) || (detail.getIstsxm().intValue() == 1)) {
        tsxmflag = true;
      }
    }
  ...
  private boolean checkCj(KqdsReg reg, List<KqdsCostorderDetail> list, HttpServletRequest request, Kqds_PayCostLogic logic, YZDictLogic dictLogic)
    throws Exception
  {
