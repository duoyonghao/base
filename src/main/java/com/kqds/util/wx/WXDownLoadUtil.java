...
public class WXDownLoadUtil {
  ...
  public static String saveImageToDisk(String mediaId, String msgid, String accessToken, HttpServletRequest request) throws Exception {
    ...
    try {
      ...
      String fullPath = String.valueOf(request.getSession().getServletContext().getRealPath("/")) + "upload" + File.separator + "wechat" + File.separator + hard + File.separator + msgid + 
        ".jpg";
      File folder = new File(String.valueOf(request.getSession().getServletContext().getRealPath("/")) + "upload" + File.separator + "wechat" + File.separator + hard);
      if (!folder.exists())
        folder.mkdirs(); 
      ...
      imageUrl = String.valueOf(imageUrl) + request.getContextPath() + "/upload/wechat/" + hard + "/" + msgid + ".jpg";
    } catch (IOException e) {
      e.printStackTrace();
    } finally {
      if (inputStream != null)
