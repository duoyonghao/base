...
public class CacheUtil {
    ...
    private static RedisTemplate redisTemplate = CacheContextUtil.getBean("redisTemplate", RedisTemplate.class);
	...
	LOG.debug(" removeZSet key :{},start:{}, end:{}", key, start);
	try {
	    if (isClose() || isEmpty(key)) {
		return false;
	    }
	...
	LOG.debug(" getZSet key :{},start:{}, end:{}", key, start);
	try {
	    if (isClose() || isEmpty(key)) {
		return Collections.emptySet();
	    }
	...
	try {
	    ...
	    key = buildKey(key);
	    //return (Set<T>) redisTemplate.opsForZSet().reverseRangeByScore(key, start, end);
	...
	public static boolean isPKexist(String key, String pk) {
        ...
        return setOperations.isMember(key, pk);
    }
    ...
    public static String queryVisitArticleStatistics(int cgfId){
    	...
    	try {
    	     ...
    	     connectionFactory.setDatabase(cgfId);//切换9号数据库
