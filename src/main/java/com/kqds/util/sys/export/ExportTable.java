...
public class ExportTable {
  public static void exportBootStrapTable2Excel(String title, String jsonNameString, String jsonValueString, List<JSONObject> dataset, HttpServletResponse response, HttpServletRequest request) throws Exception {
    ...
    HSSFComment comment = patriarch.createComment((HSSFAnchor)new HSSFClientAnchor(0, 0, 0, 0, (short)4, 2, (short)6, 5));
    ...
    try {
      ...
      response.setHeader("Content-Disposition", "attachment;filename=" + new String((String.valueOf(title) + ".xls").getBytes(), "iso-8859-1"));
  ...
  public static ExportBean initExcel4RsWrite(String title, String jsonNameString, String jsonValueString, HttpServletResponse response, HttpServletRequest request) throws Exception {
    Map<String, String> valueMap = getMapJson(jsonNameString, jsonValueString);
  ...
  public static void writeExcel4DownLoad(String title, SXSSFWorkbook workbook, HttpServletResponse response) {
    try {
      ...
      response.reset();
      response.setContentType("application/vnd.ms-excel;charset=utf-8");
      response.setHeader("Content-Disposition", "attachment;filename=" + new String((String.valueOf(title) + ".xlsx").getBytes(), "iso-8859-1"));
      ServletOutputStream out = response.getOutputStream();
      BufferedInputStream bis = null;
      BufferedOutputStream bos = null;
      try {
