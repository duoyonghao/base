...
public class PDFWriter
{
  ...
  private void createPDF(double[] orientation)
    throws Exception
  {
    this.pdf = new PDF();
  ...
  private double getMultidayHeight()
    throws Exception
  {
    String[] cols = this.parser.weekColsParsing();
  ...
  private Line Line(double[] color, double x1, double y1, double x2, double y2, Box parent)
    throws Exception
  {
    Line border = prepareLine(color, x1, y1, x2, y2);
  ...
  private Line prepareLine(double[] color, double x1, double y1, double x2, double y2)
    throws Exception
  {
    Line border = new Line();
  ...
  private void timelineContainerDraw()
    throws Exception
  {
    double[] bgColor = RGBColor.getColor(this.bgColor);
    double[] borderColor = RGBColor.getColor(this.lineColor);
  ...
  private void matrixContainerDraw()
    throws Exception
  {
    double[] bgColor = RGBColor.getColor(this.bgColor);
    double[] borderColor = RGBColor.getColor(this.lineColor);
  ...
  private void timelineEventsDraw()
    throws Exception
  {
    double[] eventBorderColor = RGBColor.getColor(this.eventBorderColor);
    double[] eventColor = RGBColor.getColor(this.eventColor);
    ...
    throws Exception
  {
    ...
    for (int i = 0; i < 2; i++)
    {
      double y = this.offsetTop + contHeight * 2.0D + tallContHeight * i;
    ...
    throws Exception
  {
    ...
    for (int i = 0; i < rest.size(); i++) {
      events_list[i] = ((SchedulerEvent)rest.get(i));
    }
    return events_list;
  }
  ...
  private void todayLabelDraw()
    throws Exception
  {
    Page p1 = (Page)this.pages.get(0);
    todayLabelDraw(p1);
  }
  
  private void todayLabelDraw(Page p1)
    throws Exception
  {
    this.f1.setSize(10.0D);
    double[] textColor = RGBColor.getColor(this.textColor);
  ...
  private int getEventIndex(SchedulerEvent[] events, int day, int week, int month)
    throws IOException
  {
    for (int i = 0; i < events.length; i++)
    {
  ...
  private void outputPDF(HttpServletResponse resp)
    throws Throwable
  {
    this.pdf.wrap();
