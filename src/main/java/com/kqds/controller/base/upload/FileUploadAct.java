package com.kqds.controller.base.upload;

import com.kqds.core.global.YZSysProps;
import com.kqds.service.base.upload.FileUploadLogic;
...
@Controller
@RequestMapping({"FileUploadAct"})
public class FileUploadAct {
  private Logger logger = LoggerFactory.getLogger(FileUploadAct.class);
  ...
  public String uploadFile(HttpServletRequest request, HttpServletResponse response) throws Exception {
    ...
    try {
      ...
      String imgtype = (String)fileForm.getParamMap().get("imgtype");
      ...
      for (String key : keys) {
        ...
        attrId = String.valueOf(attrId) + key + ",";
        attrName = String.valueOf(attrName) + value + "*";
      } 
      ...
      YZUtility.DEAL_ERROR("文件上传失败：文件需要小于" + YZSysProps.getInt("maxUploadFileSize") + "M", true, (Exception)ex, response, this.logger);
    } catch (Exception ex) {
  ...
  public void downLoad(HttpServletRequest request, HttpServletResponse response) throws Exception {
    ...
    String fileName = String.valueOf(attrId.substring(ym + 1, attrId.length() - 1)) + "_" + attrName.substring(0, attrName.length() - 1);
    String filePath = String.valueOf(request.getSession().getServletContext().getRealPath("/")) + "upload" + File.separator + module + File.separator + hard + File.separator + fileName;
