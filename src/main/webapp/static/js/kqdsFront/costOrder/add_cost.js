...
function edithz(index, field, isqfreal) {
    ...
    if (field == "paymoney") {
        if (isqfreal == 1) {
            oldValue = $("#paymoney" + index).attr("oldvalue");
            /** 这儿的情况为：一个项目欠费60，这时划价一个付款为10的单子，然后再修改这个单子，就需要取oldValue 为10的值，因为此时欠费金额为50，而不是60 **/
    ...
    if (Number(arrearmoney) < 0) {
        var oldpayvalue = $("#paymoney" + index).attr("oldvalue");
        /** 代码重复？ **/
...
function getAlljs() {
    ...
    shouldmoney += Number(payhtml) + Number(arrearhtml);
    /** 这里的应收=缴费+欠费（不包括欠费单的！） **/
