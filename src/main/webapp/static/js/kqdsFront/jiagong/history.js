...
$(function(){
	function e(){
			...
			for(i=0;4>i;i++){
				$("."+b[i]).stop(!0,!1).animate({backgroundPositionY:-24*a[i]},{duration:200});
			}
		}
		...
		(function(){
			...
			890-250<=a&&(row=8-2);
			800-250<=a&&890-250>a&&(row=7-2);
			726-250<=a&& 800-250>a&&(row=6-2);
			726-250>a&&(row=5-2)
			}
		)();
		var h=f-row;
		$(".list").height(80*row);
		$("#content").height(80*row+25);/**/
		/*$(".list").outerHeight($(window).outerHeight()-40);
		$("#content").outerHeight($(window).outerHeight()-40);*/
		$.fn.liOut=function(){
			$(this).find(".lileft").animate({left:"-400px"},500,"easeOutQuart");
			/*$(this).find(".liright").animate({right:"-700px"},500,"easeOutQuart")*/
			$(this).find(".liright").animate({left:"527px"},500,"easeOutQuart")
		};
		$.fn.liIn=function(){
			$(this).find(".lileft").animate({left:"0px"},500,"easeOutQuart");
			$(this).find(".liright").animate({left:"127px"},500,"easeOutQuart")
			/*$(this).find(".liright").animate({right:"0px"},500,"easeOutQuart")*/
		};
		$(".arrowdown").click(function(){
			f=$(".list li").length;
			h=f-row;
			a&&parseInt($(".list li:first").css("marginTop"))> -80*h&&(d++,b--,a=!1,$(".list li").eq(d-1).liOut(),$(".list li").eq(d+row-1).liIn(),$(".list li:first").animate({marginTop:80*b},600,"easeInOutQuad",function(){
				a=!0;$(".arrowdown").css("opacity",1);
				$(".arrowdown").removeClass("arrow_active")
			}),e()
		)});
		$(".arrowup").click(function(){
			a&&0!=parseInt($(".list li:first").css("marginTop"))&&(b++,d--,a=!1,$(".list li").eq(d).liIn(),$(".list li").eq(d+row).liOut(),$(".list li:first").animate({marginTop:80*b},600,"easeInOutQuad",function(){
		...
		$(".list").mousewheel(function(b,c){
			...
			if(a){
				...
				0<c?0!=d&&($(".arrowup").addClass("arrow_active"),$(".arrowup").trigger("click")):d>-80*h&&($(".arrowdown").addClass("arrow_active"),$(".arrowdown").trigger("click"))
			}
		});
		$(".timeblock").attr("thisyear",c);
		$(".list li:gt("+(row-1)+")").find(".lileft").css({left:"-400px"});
		$(".list li:gt("+(row-1)+")").find(".liright").css({left:"527px"});
