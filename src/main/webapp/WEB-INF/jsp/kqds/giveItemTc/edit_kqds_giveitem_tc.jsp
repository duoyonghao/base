...
<title>编辑赠送套餐</title> <!-- 由费用添加页面  新增套餐， 后台提供编辑 查询 功能 -->
...
<style type="text/css">
   li{color:#00A6C0;cursor: pointer;}
  	.aBtn{display:inline-block;padding: 0 15px;height: 28px;border: solid 1px #0e7cc9;color: #0e7cc9;border-radius: 15px;line-height: 28px;width: 60px;text-align: center;}
	.aBtn:hover{color:#fff;background: #0e7cc9;}
		...
		<div class="ztreeWrap">
			<p class="zsxmText">添加赠送项目</p>	
			 <div class="ztree" style="overflow-y:auto;">
				 <ul id="zsitems" style="height:290px;overflow:auto;">
				 </ul>
			 </div>
		</div>
...
<div style="text-align: left;width:50%;margin-top: 10px;float: left;">
	...
	<select name="useflag" id="useflag"  style="width:40px;height:20px;line-height:20px; border-radius:3px;color:#333; border:1px solid #ddd;" id="useflag"  style="width: 50px;">
...
var pageurl = contextPath + '/KQDS_GiveItem_TCBackAct/getItemsByTcno4Back.act?seqId=' + seqId;
$(function () {
	$("#name").val(name);
	$("#useflag").val(useflag);
	inititems();//加载左侧项目列表
        ...
        function(data){
			...
			if(tabledata.length > 0){
				...
				for(var i = 0; i < tabledata.length;i++){
					...
					//seqId
					tablehtml += '<td style="display:none;">'+tabledata[i].memberno+'</td>';//seqId保存在memberno里了 特殊情况
					//项目编号
					tablehtml += '<td style="display:none;">'+tabledata[i].itemno+'</td>';
					//项目名称
                	tablehtml += '<td><span style="text-align:left;width:25em;">' + tabledata[i].itemname+ '</span></td>';
                	//单位
                	tablehtml += '<td style="text-align:center;"><span class="name">' + tabledata[i].unit+ '</span></td>';
                	//单价
                	tablehtml += '<td style="display:none;">'+tabledata[i].unitprice+'</td>';
                	//数量
                	tablehtml += '<td><input type="number" min="1" style="width:70px;text-align:center;padding-left:10px;border:1px solid #ddd;" onfocus="this.select()" name="zsnum" id="zsnum'+tdindex+'" value="'+tabledata[i].zsnum+'"></td>';
	...
	$.axse(url,null, 
		function(data){
			if(data.retState=="0"){
            	...
            	//数量
            	tablehtml += '<td><input type="number" min="1" style="width:70px;text-align:center;padding-left:10px;border:1px solid #ddd;" onfocus="this.select()" name="zsnum" id="zsnum'+tdindex+'" value=1></td>';
				tablehtml += "</tr>";
...
function baocun(){
	...
	var name = $("#name").val();
	if(name == ""){
		layer.alert('请填写套餐名！', {  end:function(){
			//选中套餐名称input
			$("#name").focus();
		}});
		return false;
	...
	var url = '<%=contextPath%>/KQDS_GiveItem_TCBackAct/saveGiveTc.act?seqId=' + seqId + '&params='+encodeURIComponent(data)+'&name='+name + '&useflag=' + useflag;
    $.axse(url,null, 
         function(data){
		     if(data.retState=="0"){
     		     layer.alert('保存成功', {
