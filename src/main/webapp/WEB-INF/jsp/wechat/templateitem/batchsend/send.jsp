...
<div class="container"><!--提供内边距 让内容不顶头显示-->
	...
	<div class="fixedBottomDiv"><!--底部三个按钮所在父元素 固定在底部样式  -->
		<div class="clear2"></div>
        	<a class="kqdsCommonBtn" id="submit">发送</a>
	</div>
</div>
</body>
...
$(function() {
    if (templateSeqid && templateId && seqId) {
        intInfo();
    }
});
...
// 模板消息
function getDetail() {
	var detailurl = "WXTemplateItemAct/selectDetail.act?seqId=" + seqId;
    var serverData = getDataFromServer(detailurl);
    if (serverData) {
...
function submitForm() {
    ...
    var url = "WXReceiveTextAct/sendTemplateBath.act?";
    url += param;
    url += "&templateId=" + templateId + "&templateSeqid=" + templateSeqid;
    
    var parentSelectOpenIds = parent.parent.static_local_templateobj.openIds;
    
    var parentParam = {
    		openIds:parentSelectOpenIds
    }
    
    var rtData = getDataFromServer(url, parentParam);
    if (rtData.retState == 0) {
        layer.alert(rtData.retMsrg, {
              
            end: function() {
            	parent.parent.static_local_templateobj = new Object();
            	parent.refreshWindow(rtData);
            }
        });
    }
}
