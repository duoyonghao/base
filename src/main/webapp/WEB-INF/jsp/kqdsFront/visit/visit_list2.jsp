	...
	<div>	
	    ...
	    <div class="searchModule" style="margin-bottom:2px;text-align:right;">
	    	...
	    	<section id="menzhen" style="border-bottom:none;">
	    		...
	    		<ul class="conditions">
					...
					<li>
						<span>模糊查询</span>
						<input type="text" id="musername" name="musername"  placeholder="患者姓名" />
					</li>
					<li>
						<span>回访分类</span>
		...
		<div class="buttonBar" style="text-align:left;" >
			...
			<%
				if (is_Wd_oper == null || "".equals(is_Wd_oper)) {
			%>
					<a href="javascript:void(0);" id="tjhfjh" class="kqdsCommonBtn" onclick="goAddVisitPlan()">添加回访计划</a>
					<%
						}
					%>
...
var onclickrowObj = "";
var pageurl = '<%=contextPath%>/KQDS_VisitAct/selectListByPersonIdAndTime.act';
...
function goAddVisit() {
    ...
    if (onclickrow.usercode == null || onclickrow.usercode == "") {
        layer.alert('请选择患者'  );
        return false;
    }
    layer.open({
        ...
        shade: 0.6,
        area: ['550px', '480px'],
        content: '<%=contextPath%>/KQDS_VisitAct/toVisitAdd.act?lytype='+lytype+'&usercode=' + onclickrow.usercode + '&regno=' + regno //iframe的url
    });
}
...
function goAddVisitPlan() {
    ...
    parent.layer.open({
        ...
        shade: 0.6,
        area: ['850px', '580px'],
        content: '<%=contextPath%>/KQDS_VisitAct/toVisitPlanAdd.act?lytype='+ lytype+'&usercode=' + onclickrow.usercode + '&regno=' + regno  //iframe的url
    });
}
...
function query() {
    ...
    /* console.log( $('#table').bootstrapTable('refresh', {
        'url': pageurl
    })) */
}
