    ...
    <table id="table">
		<tbody>
			<tr class="marg">
				...
				<td>
					<select class="hfdept visit" name="hfdept" id="hfdept" onchange="getVisitPersonSelect('hfdept','visitor');"></select>
				</td> 
				<td>
...
<!-- 				                    <th class="operateWidth">操作</th> -->
			...
			<tr>
				<td colspan="6" style="text-align:center;">
				 	<button class="planBtn smallPlanAdd disAbledBtn"  disabled="disabled"  onclick="addSmallPlan()">添加</button>		
...
function checkIdadmin (){
	 var url = contextPath + '/HUDH_AddVisitAct/jurisdiction.act';
	    $.axse(url, null,
	    function(data) {
	    	if(!(data.valid)){
...
//  			return;
			var url = '<%=contextPath%>/KQDS_VisitAct/insertOrUpdate2.act';
        	...
        	function(r){
    	        if(r.retState=="0"){
    	        	...
    	        	layer.alert('添加失败：' + r.retMsrg );contentWindow
    	        }
            },
            function(){
          	    layer.alert('添加失败' );
