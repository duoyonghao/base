...
<div id="container">
    ...
    <div class="tableHd">医生材料成本统计</div>
    	...
    	<div class="tableHeader" style="position:absolute; top:0;left:0;height:55px;overflow:hidden;">
    		<table id="tableHeader" class="table-striped table-condensed table-bordered" style="visibility:hidden;">
     	 		<tr>
			 		...
			 		<td  style="text-align: center">材料费占比</td>
			 		<td  style="text-align: center">加工费</td>
			 		<td  style="text-align: center">加工费占比</td>
		 		</tr>
...
var pageurl = contextPath+'/KQDS_ScbbAct/selectCountClcb.act';
var nowday;
var menuid = "<%=menuid%>";
...
function gettabledata(){
	...
	var url = pageurl+"?starttime="+starttime+"&endtime="+endtime+"&depttype=1&organization="+$("#organization").val();
	 $.axse(url,null, 
             function(data){
			    data = data.rows;
		 		if(data!=null){
		 			if(data.length>0){
		 				...
		 				for(var i=0;i<data.length;i++){
		 					if(data[i].username){
			 					...
			 					content+='<td style="text-align: center">'+data[i].jgje+'</td>';
			 					content+='<td style="text-align: center">'+data[i].jgzb+'</td>';
			 					content+='<td style="text-align: center">'+data[i].ckje+'</td>';
			 					content+='<td style="text-align: center">'+data[i].ckzb+'</td>';
			 					content+='</tr>';
		 					}else{
		 						content+='<tr>';
		 						content+='<td style="text-align: center">合计</td>';	
		 						content+='<td style="text-align: center">'+data[i].ssje+'</td>';
			 					content+='<td style="text-align: center">'+data[i].jgje+'</td>';
			 					content+='<td style="text-align: center">'+data[i].jgzb+'</td>';
			 					content+='<td style="text-align: center">'+data[i].ckje+'</td>';
