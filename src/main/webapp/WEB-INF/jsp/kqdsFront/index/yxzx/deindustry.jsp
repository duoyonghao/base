							...
							<table class="formBox tableLayout">
								<tbody>
									<tr>
										...
										<td>
											<span class="commonText" style="color:red;">企业名称：</span>
										</td>
										<td>
											<input type="text" class="zxInput" placeholder="企业名称" id="qyname" name="qyname">
										</td>
									</tr>
									...
									<tr>
										...
										<td>
											<span class="commonText" style="color:red;">异业投入：</span>
										</td>
										<td>
											<input type="text" class="zxInput" style="width: 180px;" placeholder="异业投入" id="outmoney" name="outmoney">
									...
									<tr>
										<td>
											<span class="commonText">客户经理：</span>
										</td>
										<td>
											<input type="hidden" name="activitycontacts" id="activitycontacts" class="form-control" /> 
											<input style="cursor:pointer;" type="text" id="activitycontactsDesc" name="activitycontactsDesc" placeholder="客户经理" onClick="javascript:multi_select_user(['activitycontacts', 'activitycontactsDesc']);"></input>
										</td>
										<td>
											<span class="commonText">是否合作：</span>
										</td>
										<td>
											<input type="checkbox" name="ishz" id="ishz" value="合作">
										</td>
									</tr>
									...
									<tr>	
										...
										<td colspan="3">
											<textarea class="longTextarea" name="cooperationtype" id="cooperationtype" rows="3" placeholder=""></textarea>
										</td>
									</tr>
									<tr>	
										<td>
	...
	//初始化表格所在div
    	$('#table').bootstrapTable({
    	    ...
    	    columns: [
		                ...
		                {title: '企业名称',field: 'qyname',align: 'center',sortable: true,
		                	  formatter:function(value,row,index){  
			                	  if (value) {
		                ...
		                {
		                	title: '是否合作',field: 'ishz',align: 'center',sortable: true,
		                	formatter:function(value){
		                		return '<span>'+value+'</span>';
		                	}
		                	
		                },
		                {title: '地址',field: 'address',align: 'center',sortable: true,
		                	  formatter:function(value,row,index){  
			                	  if (value) {
			                          return '<span class="remark" title="' + value + '">' + value + '</span>';
			                      }
						...
						{title: '客户经理',field: 'activitycontactsname',align: 'center',sortable: true,
							 formatter: function(value) {
					            	return '<span class="name" title="' + value + '">' + value + '</span>';
					         }
						},
						{title: '活动内容',field: 'cooperationtype',align: 'center',sortable: true,
			                  formatter:function(value,row,index){  
			                	  if (value) {
			                          return '<span class="remark" title="' + value + '">' + value + '</span>';
			                      }
...
function getdata(seqId){
	var detailurl = '<%=contextPath%>/Kqds_DeindustryRecordAct/selectDetail.act?seqId=' + seqId;
	$.axse(detailurl, null,
			function(data) {
				  loadData(data.data);
  ...
  function save(){
    ...
    var ishz = $("[name='ishz']:checked").val();
	if(ishz == null || ishz == "undefined"){
		param += "&ishz=" ;
	}
  	var editurl = '<%=contextPath%>/Kqds_DeindustryRecordAct/insertOrUpdate.act?'+param;
	$.axse(editurl,null, 
              function(data){
				  if(data.retState=="0"){
					 layer.alert('保存成功！' );
...
function getButtonPower(){
    ...
    for (var i = 0; i < listbutton.length; i++) {
        if (listbutton[i].qxName == "bc") {
            ...
            $("#filePicker").show();
        }else if(listbutton[i].qxName == "yyjl_zxqk"){
        	menubutton2 += ' <a href="javascript:void(0);" class="kqdsCommonBtn" onclick="zxqk(\'KQDS_DEINDUSTRYRECORD\');">咨询情况</a> ';
        }else if(listbutton[i].qxName == "yyjl_skmx"){
        	menubutton2 += ' <a href="javascript:void(0);" class="kqdsCommonBtn" onclick="skmx();">收款明细</a> ';
        }else if(listbutton[i].qxName == "yyjl_user"){
        	menubutton2 += ' <a href="javascript:void(0);" class="kqdsCommonBtn" onclick="xxcx();">患者明细</a> ';
        }else if(listbutton[i].qxName == "yyjl_scbb"){
...
function setHeight() {
   var baseHeight = $(window).height()-15,
    ...
    $('#table').bootstrapTable('resetView',{height:baseHeight - optAreaHeight - 3});
}
</script>
</html>