  ...
  <div id = "fytjDiv"><!-- ###费用添加start -->	
	<div class="costWrap">
		<div class="costHd"> <!-- Header -->
			<label class="sfbz"><input type="radio" name="treetype" id="sfbz" checked="checked">收费标准</label>
			<label class="sftc"><input type="radio" name="treetype" id="sftc">收费套餐</label>
...
$(function() {
	...
	if(developArr.length>0&&status==1&&developArr!=''){
		tdindex=tdindex+developArr.length;
	}
	if(developSpaceArr.length>0&&status==2&&developSpaceArr!=''){
		tdindex=tdindex+developSpaceArr.length;
	}
	if(list!=null){
		if(list.length>0&&status==1&&developArr.length==0){
			tdindex=tdindex+list.length;
		}
		
	}
	if(list!=null){
		if(list.length>0&&status==2&&developSpaceArr.length==0){
			tdindex=tdindex+list.length;
		}
	...
	if(list!=null){
		if(list.length>0&&status==1&&developArr==''){
	    	addPriceListDetail1(list);
		}
	}
	if(list!=null){
		if(list.length>0&&status==2&&developSpaceArr==''){
			addPriceListDetail1(list);
		}
...
function addPriceListDetail1(list){
	tdindex = 1000;
	 //显示条数
	var tablehtml = "";
	 if(list!=null){
...
//	        		   console.log(JSON.stringify(r)+"----------r");
	        		   ...
	        		   var needAlreadyHtml=$(window.parent.frames["tabIframe"].document).find("#needShow").html();
		   				if(needAlreadyHtml.indexOf("li")!=-1){
		   					$(window.parent.frames["tabIframe"].document).find("#needShow").find("li").each(function(i,ele){
		   						...
		   						if($(ele).attr("slid")=='61223947-c0da-4d0f-a1ff-429232d10a3c' && r.status==1){
		   							...
		   							var newLabelValue=r.leveLabel+'<span onclick="cleatBtn(this)" id="cleatBtn" class="cleatBtn">X</span>';
		   							$(window.parent.frames["tabIframe"].document).find("#needShow").find("li").eq(i).attr("value",r.leveLabel);//标签名称
		   							$(window.parent.frames["tabIframe"].document).find("#needShow").find("li").eq(i).html(newLabelValue);//标签内text值
		   							$(window.parent.frames["tabIframe"].document).find("#needShow").find("li").eq(i).attr("id",r.seqid);//标签id
		   							$(window.parent.frames["tabIframe"].document).find("#needShow").find("li").eq(i).attr("privelist",JSON.stringify(r.priveListDetails));//标签存储数据privelist
		   				...
		   				if(developPIndex>=0){
		   					...
		   					//向个人信息页面追加标签
		   					var needHtml=needAlreadyHtml+'<li id='+r.seqid+' slid='+parentId+' slname='+parentName+' class="aa" value='+r.leveLabel+' privelist='+JSON.stringify(r.priveListDetails)+'>'+r.leveLabel+'<span onclick="cleatBtn(this)" id="cleatBtn" class="cleatBtn">X</span></li>';
