...
<!-- 						</select> -->
						<select class="patients-source select2 dict searchSelect" source="hzly" name="devchannel" id="devchannel" onchange="getSubDictSelectSearch('devchannel','nexttype', 'add');" data-live-search="true" title="请选择">
						</select>
					</div>
				</td>
				<td>
...
<!-- 						</select> -->
						<select name="nexttype" class="searchSelect" id="nexttype" data-live-search="true" title="请选择">
						</select>
					</div>
				</td>
			</tr>
...
<!-- 						</select> -->
						<select class="dict searchSelect" tig="SLGJ" name="accepttool" id="accepttool" data-live-search="true" title="请选择"></select>
					</div>
				</td>
			</tr>
...
$(function(){
	...
	initStreet();
	/** ########################################################### 连锁相关  **/
	initHosSelectListNoCheck('organization',static_organization); // 连锁门诊下拉框
...
//   $("#town").val("110101001").trigger("change");
 	...
 	initDictSelectByClassSource(deptId);  // 患者来源分离出来做部分隐藏(本页面专用)
 ...
 * 请求方式为 同步
 */
function initDictSelectByClassSource(deptId,operFlag) {
    if ($(".dict").length > 0) {
