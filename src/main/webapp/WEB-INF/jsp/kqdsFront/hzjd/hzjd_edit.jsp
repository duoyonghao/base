			...
			<table class="tableContent"><!--tableContent的样式 -->
				<tbody>
					...
					<tr>
						...
						<td>
							<div class="form-group">
								<select class="sel_short_inp_long" id="familyship" name="familyship">	<!--.sel_short_inp_long select与input组合时的样式  -->
					...
					<tr>
						<td>
							<span class="comText">病历号</span>
						</td>
						<td>
							<div class="form-group">
								<input type="text" id="blcode" name="blcode" placeholder="" readonly="readonly" onclick="alert('没有权限修改病历号');"/>
							</div>
						</td>
					</tr>
					...
					<tr>
						...
						<td class="relative">
							<div class="form-group">
								<select class="select2 dict searchSelect" resource="hzly" name="devchannel" id="devchannel" data-live-search="true" title="" onchange="getSubDictSelectSearchfilter(existornot,'devchannel','nexttype','add');"></select>
<!-- 								<select class="select2 dict" tig="hzly" name="devchannel" id="devchannel" onchange="getSubDictSelect('devchannel','nexttype','add');"></select> -->
						...
						<td>
							<div class="form-group">
								<select id="nexttype" name="nexttype" class="select2 dict searchSelect"  data-live-search="true" title=""></select>
					...
					<!-- <tr>
						...
						<td colspan="3">
							<div class="form-group provinceReg">.provinceReg 验证信息位置 调整样式
								<select class="sel3" style="width:98px" id="province" name="province"></select> 
								<select class="sel3" style="width:98px" id="city" name="city" ></select>
				 ...
				 <td>
					...
					<td colspan="3">
						<div class="form-group provinceReg">
							<select id="province" name="province" class="sel3 searchSelect"  data-live-search="true"  onchange="initCity()"></select>
							<select id="city"  name="city" class="sel3 searchSelect"  data-live-search="true"   onchange="initArea()"></select>
							<select id="area"  name="area" class="sel3 searchSelect"  data-live-search="true"  onchange="initStreet()"></select>
							<select id="town"  name="town" class="sel3 searchSelect"  data-live-search="true" title="请选择" ></select>
					...
					<tr>
						...
						<td>	<!-- .whiteInp  input白底鼠标移入的效果-->
							<input type="hidden" name="createuser" id="createuser"  class="form-control" />
				 			<input type="text" class="whiteInp" id="createuserDesc" name="createuserDesc" placeholder="建档人"  onClick="javascript:single_select_user(['createuser', 'createuserDesc']);"  ></input>	
						</td>
	...
	connect();
	 $(window).on('load', function () {
       ...
       // $('.selectpicker').selectpicker({});
        $(".input-block-level").attr('placeholder','');
    });
	prov();
	initCity();
	initArea();
	var tmpUserObj = getBaseInfoByUserCode(usercode); // 这里有特殊性，要在加载详情前，先给Organization赋值，这样做是为了取到对应的患者来源子分类
	lastnexttype=tmpUserObj.nexttype
//     var hobbys = (tmpUserObj.hobby).split(";");
//     var select = $("#hobby");  
//     for (var i = 0; i < hobbys.length; i++) {  
//         select.append("<option value='"+hobbys[i]+"'>" + hobbys[i] + "</option>");  
//     }  
//     $('.selectpicker').selectpicker('val', '');  
//     $('.selectpicker').selectpicker('refresh');  
	
	$("#organization").val(tmpUserObj.organization);
	
    //国家sel
  
	/* showLocation();//省市级联c */
	initDictSelectByClassIscustomer(existornot); // 患者来源前台专用
...
//    $('#hobby').selectpicker('val')
//    console.log(tmpUserObj)
//     var fatherArr = []
//     var childArr = []
//     var hobbyall =  $("#hobby option")
//     for(var i=0;i<hobbyall.length;i++){
//     	fatherArr.push(hobbyall[i])
//     }
//     childArr = tmpUserObj.hobby.split(";")
//     console.log(childArr,"aaasosososo")
//     for(var i =0;i<fatherArr.length;i++){
//     	for(var j=0;j<childArr.length;j++){
//     		if($(fatherArr[i]).text()!=childArr[j]){
//     			continue;
//     		}else if($(fatherArr[i]).text() == childArr[j]){
//     		   $($(".dropdown-menu li")[i]).addClass("active selected")
//     		   console.log($(".dropdown-menu li")[i],"1111111")

//     		   console.log(document.getElementById('content'),"sssssssssdddddddddd")
...
//     $('.btn').selectpicker('val', tmpUserObj.hobby);
    submit(); //submit()校验方法
    initStreet(tmpUserObj.area);
//     $("#province").val(tmpUserObj.province);
//     $("#city").val(tmpUserObj.city);
//     $("#area").val(tmpUserObj.area);
//     $("#town").val(tmpUserObj.town);
	$('.searchSelect').selectpicker("refresh");//初始化刷新搜索--2019.10.11--licc
    $("#town").selectpicker("val",tmpUserObj.town);
});
 ...
 */
function getSubDictSelectSearchfilter(existornot,parentSelectId, selectId, isAdd) {
        ...
        if (data.retState == "0") {
            ...
            if (list != null && list.length > 0) {
                for (var j = 0; j < list.length; j++) {
                    var optionStr = list[j];
                    if(existornot){
	                      if(optionStr.parentCode=="sc569"||optionStr.parentCode=="zrdz510"||optionStr.parentCode=="lpyjs106"||optionStr.parentCode=="qdzz13"||optionStr.parentCode=="ptlz562"||optionStr.parentCode=="pyjs900"||optionStr.parentCode=="nbygkf734"||optionStr.parentCode=="yyyg526"||optionStr.parentCode=="wzqdjs66"){
...
function noEdit(){
	if(canEditPhone != 1){
		if($("#phonenumber2").val()!=""){
			$("#phonenumber2").attr("readonly","readonly").click( eval(function(){
	    		alert('无修改权限！');
	    	})); 
		}
		if($("#phonenumber1").val()!=""){
			$("#phonenumber1").attr("readonly","readonly").click( eval(function(){
	    		alert('无修改权限！');
	    	})); 
		}
	}
...
//是否他人介绍切换联动介绍人
$("#introduce").change(function(){
	 var or = $("#introduce").val();
...
function readCert() {
    ...
    if (result == "") {
        ...
        if (resultObj.resultFlag == "0") {
            ...
            effDate = resultObj.resultContent.effDate;//发证日期
            expDate = resultObj.resultContent.expDate;//证件最后失效时间
