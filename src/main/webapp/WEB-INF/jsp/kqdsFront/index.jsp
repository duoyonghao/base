...
<head>
    ...
    <meta charset="utf-8"/>
    <link rel="shortcut icon" href="<%=contextPath%>/static/image/image/icon/favicon.ico"
          mce_href="<%=contextPath%>/static/image/image/icon/favicon.ico" type="image/x-icon">
    ...
    <div id="header">
        <div class="infoBox">
            ...
            <div class="nowstate" style="margin-right:10px;">
                <span id="nowstate" class="nowstate" data-value="0"
                      style="display:inline-block;margin-right:0;cursor:pointer;margin-left: 3px;">空闲</span>
            ...
            <ul id="userul" style="margin:0px;">
                ...
                <li onclick="todaywork();">
                    <span class="LvUpTopIcon icon20 todaywork-tip-icon"
                          style="margin-right:0;position:relative;top:4px;"></span>今日工作
                </li>
                <li onclick="openPushList();">
                    <span class="LvUpTopIcon icon20 system-tip-icon"
                          style="margin-right:0;position:relative;top:4px;"></span>
                ...
                <li onclick="showOnlinePersonDiv();" style="position:relative;">
                    <span class="icon20 onlinePersonIcon"
                          style="margin-right:0;position:relative;top:6px;right:3px;"></span>
                    <span id="userCountInput" class="notRead">12</span>
                    在线人数

                </li>
                <li onclick="doLogout();">
                    <span class="LvUpTopIcon icon20 exit-icon"
                          style="margin-right:0;position:relative;top:6px;right:3px;"></span>退出
                </li>
            </ul>
            ...
            <div id="userMeassageDiv" class="userMeassageDiv" style="display:none;font-size:14px;">
                <img style="width:20px;height:20px; position:absolute;top:-16px;right:240px;"
                     src="<%=contextPath%>/static/image/kqdsFront/img/icon/white_arrow.png"/>
                <img src="<%=contextPath%>/static/image/kqdsFront/img/closeGray.png" class="closeBtn"></i>
                <div class="H42Div">
                    <div class="left"
                         style="text-align:right;line-height:22px;height:22px;font-size:22px;padding-right: 12px;">
                        <%=userName%>
                    </div>
                ...
                <form id="updataPasswordForm" class="updataPasswordForm" style="padding:20px 0;">
                    ...
                    <div style="padding-top:15px;text-align:center;">
                        <div class="right" style="height:30px;line-height:30px;">
                            <button id="submitUpdataPasswordBtn" type="button" class="kqdsSearchBtn"
                                    style="height:30px;line-height:30px;font-size:16px;width:100px;">确 定</a>
                        </div>
                    </div>
                </form>
            </div>
        ...
        <div class="header-main">
            ...
            <div id="kqdsInformationDiv" class="kqdsInformationDiv" style="display:none;">
                <img style="width:20px;height:20px; position:absolute;top:-16px;left:60px;"
                     src="<%=contextPath%>/static/image/kqdsFront/img/icon/white_arrow.png"/>
                <img src="<%=contextPath%>/static/image/kqdsFront/img/closeGray.png" class="closeBtn"></i>
                <h2 style="margin:0;height:40px;padding:0 0 20px 20px;border-bottom:1px solid #ddd;font-size:20px;">
                    关于UNIC大数据管理系统</h2>
                <div style="padding:20px;border-bottom:1px solid #ddd;">
                    ...
                    <div style="padding:30px 0 20px;">
                        ...
                        <p style="margin:0;color:#00A6C0;">官方网址：http://www.hdbkq.cn<a style="color:#00A6C0;"
                                                                                      href="http://www.hdbkq.cn"></a>
                        </p>
                    </div>
                </div>
        ...
        <div id="onlinePersonDiv" class="onlinePersonDiv" style="display:none;">
            <img style="width:20px;height:20px; position:absolute;top:-16px;right:123px;"
                 src="<%=contextPath%>/static/image/kqdsFront/img/icon/white_arrow.png"/>
            <img src="<%=contextPath%>/static/image/kqdsFront/img/closeGray.png" class="closeBtn"></i>
                        ...
                        <li>
                            <c:choose>
                                <c:when test="${'[]' != menu2.hasmenu}">
                                    <div class="item">
                                        <img class="image-icon ${menu2.icon}"
                                             src="<%=contextPath %>/static/image/kqdsFront/img/icon/${menu2.icon}.png"/>${menu2.menuname}
                                    ...
                                    <div class="subChild">
                                        <ul>
                                            <c:forEach items="${menu2.hasmenu}" var="menu3" varStatus="status2">
                                                <li target="iframe"
                                                    url='<%=contextPath %>${menu3.url}?menuId=${menu3.menuid}'>
                                ...
                                <c:otherwise>
                                    <c:choose>
                                        <c:when test="${fn:contains(menu2.menuname,'首页')}">
                                            <div class="item" style="display:none;" id="index_sy"
                                                 onclick="getmenubuttonParent('${menu2.url}?menuId=${menu2.menuid}')">
                                                <span id="sy_menuid">${menu2.menuid}</span>
                                                <img class="image-icon ${menu2.icon}"
                                                     src="<%=contextPath %>/static/image/kqdsFront/img/icon/costDetailIcon.png"/>
                                                <!-- 这儿随便写个存在的图片就行 -->
                                                    <%-- <span class="lvUp-icon icon20 ${menu2.icon}"></span>${menu2.menuname} --%>
                                            </div>
                                        </c:when>
    ...
    <div id="mainWrap">
        ...
        <div class="mainContainer">
            <iframe id="iframe" name="iframe" src="" frameborder="0"
                    width="100%" height="100%" class="mainFrame" allowfullscreen="true"
...
<div class="searchPanle" style="display: none; padding-top:25px;">
    <img alt="关闭" src="<%=contextPath%>/static/image/kqdsFront/img/closeGray.png" onclick="closeInfoDiv();"
         id="closeInfoDiv">
...
<script type="text/javascript">
    ...
    function checkUserOnlineStatus() {
        ...
        // 轮训调用下一次检查
        onlineMsrgTimer = setTimeout(checkUserOnlineStatus, checkTimeout * 15000);
    }
...
//弹出/关闭用户当前状态选择框
    ...
    function doInit() {  //初始化在线人员
        ...
        var onlineRef = 120;
        getOnlinePerson();
        setInterval(getOnlinePerson, onlineRef * 15000);
    }
