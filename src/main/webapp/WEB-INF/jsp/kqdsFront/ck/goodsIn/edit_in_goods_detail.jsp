        ...
        <table id="table" class="table-striped table-condensed table-bordered" style="width: 100%">
			<thead >
						<tr>
							...
							<th style="text-align: center; vertical-align: middle;">所属仓库</th>
							<th style="text-align: center; vertical-align: middle;">一级类别</th>
							<th style="text-align: center; vertical-align: middle;">二级类别</th>
							...
							<th style="text-align: center; vertical-align: middle;">批号</th>
							<th style="text-align: center; vertical-align: middle; width:100px">注册证号</th>
...
function addDoodsDetail(){
	//console.log("回显数据="+JSON.stringify(goodsinDetail));
		var tablehtml = "";
		tablehtml += "<tr style=''>";
		...
		//类别1
		tablehtml += '<td style=";">' + goodsinDetail.firsttype + '</td>';
		//类别2
		tablehtml += '<td style=";">' + goodsinDetail.goodstypename + '</td>';
		//规格3
		tablehtml += '<td style=";">'+goodsinDetail.goodsnorms+'</td>';
		//单位4
		tablehtml += '<td style=";">'+goodsinDetail.goodsunit+'</td>';
		//单价5
		tablehtml += '<td style=";width:80px;"><span id="inprice">'+goodsinDetail.inprice+'</span></td>';
		//数量6
		tablehtml += '<td style=";width:80px;"><input type="text" style="width:100%; text-align:center;" onchange="editPrice();" onfocus="this.select()" name="innum" id="innum" oldvalue='+goodsinDetail.innum+' value='+goodsinDetail.innum+'></td>';
		//金额7
		tablehtml += '<td style=";width:80px;"><input type="text" style="width:100%;text-align:center;" onchange="editPrice();" onfocus="this.select()" name="rkmoney" id="rkmoney" oldvalue='+goodsinDetail.rkmoney+' value='+goodsinDetail.rkmoney+'></td>';
		//有效期8
		tablehtml += '<td style=";width:150px;"><input type="text" style="width:100%; text-align:center;" class="unitsDate" name="yxdate" id="yxdate" value='+goodsinDetail.yxdate+'></td>';
		//批号9
		tablehtml += '<td style=";width:150px;"><input type="text" style="width:100%; text-align:center;" name="ph" id="ph" value='+goodsinDetail.ph+'></td>';
		//注册证号14
	    tablehtml += '<td style="width:100px;"><input type="text" style="width:100%; text-align:center;" onfocus="this.select()" name="zczh" id="zczh" value="'+goodsinDetail.zczh+'" ></td>';
		//入库备注10
		tablehtml += '<td style=";width:150px;"><input type="text" style="width:100%; text-align:center;" onfocus="this.select()" name="sqremark" id="sqremark" value='+goodsinDetail.sqremark+'></td>';
		//商品主键11
...
function editPrice(){
	 ...
	 if(innum!="" && rkmoney!=""){
		 if(Number(innum)==0){
			 ...
			 var inprice = (rkmoney/innum).toFixed(3);
...
//  var url = contextPath + '/KQDS_Ck_Goods_InAct/editGoodsIn.act';
    var url = contextPath + '/KQDS_Ck_Goods_InAct/editGoodsInDetail.act';//修改入库明细
    $.axse(url, paramOrder,
    function(r) {
        if (r.retState == "0") {
            layer.alert('操作成功', {
...
//		    var url = contextPath + '/KQDS_Ck_Goods_InAct/deleteGoodsIn.act';
		    var url = contextPath + '/KQDS_Ck_Goods_InAct/deleteGoodsInDetailById.act';
		    $.axse(url, paramOrder,
		    function(r) {
		        if (r.retState == "0") {
		            layer.alert('操作成功', {
		    ...
		    //删除入库信息的同时，保存删除人的信息
		    $.ajax({
				url: contextPath + "/HUDH_Ck_Goods_In_DeleterAct/saveDeletePersonInfor.act",
				type:"POST",
				dataType:"json",
				async: true,
				data:{
					inseqId : goodsin.seqId,//入库单主键
					indetailseqId : goodsinDetail.seqId,//入库明细主键
				    goodsseqId : $("#gseqId").val(),//商品结存
				    remark :  $("#sqremark").val(),
				    goodsname : goodsinDetail.goodsname,
				    goodscode : goodsinDetail.goodscode,
				    goodsnorms : goodsinDetail.goodsnorms
				},
