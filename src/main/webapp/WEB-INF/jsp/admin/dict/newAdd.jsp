...
/*     if (dictCode == "") {
    ...
    if("YYXM" == static_parentCode){ // 门诊预约的预约项目
		if(remark){
			var testz = /^\d+(\.\d+)?$/;
	    	if(!testz.test(remark)){
	    		layer.alert('默认预约时间需为整数'  );
	    		return false;
	    	}
	    	if(Number(remark) % 15 != 0){
	    		layer.alert('默认预约时间需为15的倍数'  );
	    		return false;
	    	}
		}
	}
    if("HYKFL" == static_parentCode){
    	if(remark==''){
    		$("#remark").val('100');
		}
    }
