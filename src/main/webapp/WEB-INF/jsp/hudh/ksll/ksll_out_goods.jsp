...
<!-- 			    			<td style="text-align:right;" readonly="readonly">出库仓库：</td> -->
<!-- 			    			<td style="text-align:left;"> -->
...
<!-- 			    			</td> -->
			    		...
			    		<!-- <tr>
			    			<td colspan="8" align="center">
						            ...
						            <a href="javascript:void(0);" onclick="save()" class="kqdsCommonBtn">保&nbsp;&nbsp;存</a>
			    			</td>
			    		</tr> -->
			    	</table>
...
function addDoodsDetail(detail){
	//console.log("科室出库="+JSON.stringify(detail));
     ...
     //库存数量9
     tablehtml += '<td style="width:80px;"><span id="nums'+tdindex+'">'+detail.nums+'</span></td>';
   	 //领料数量10
     ...
     //商品主键14
 	 ...
 	 //结存单价18
 	 tablehtml += '<td style="display:none;"><span id="price'+tdindex+'">'+detail.goodsprice+'</span></td>';
 	 //下标19
     ...
     if(outtype =="3" || outtype=="5"){
   	...
   	//根据出库数量和编号更改
   	//出库数量
	//var outnums = $("#outnum"+tdindex).val();
	//商品id
	//var goodsid=$("#goodsid"+tdindex).html();
	...
	//$.ajax({
     ...
     //    dataType: "json",
     //    success: function(r){
     //   	 $("#ckmoney"+tdindex).html(r.ckmoney);
    //    	 $("#goodsprice"+tdindex).val(r.goodsprice);
    //     }
    // });
}
...
function initChukuNum(tdindex,goodsid){
	...
	var url = contextPath + "/KQDS_Ck_Goods_In_DetailAct/selectAllGoodPhByGoodCode.act?type=1";
	var param = {
		goodsuuid : goodsid
	};
    $.axse(url, param,
...
function checknums(id,i){
	if(a==1){
		...
		var url = contextPath + '/KQDS_Ck_Goods_OutAct/findGoodsprice.act?type=1';
...
//     }
    ...
    } else if(outtype == 3 || outtype == 5){//换货
...
function initKsllData(){
	$.ajax({
		url: contextPath + '/HUDH_KSllAct/findKsllColorAndDetails.act',
		type:"POST",
		dataType:"json",
