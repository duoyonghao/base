    	...
    	<form class="form-horizontal" id="form1" style="border-bottom:1px solid #ddd;">
	        ...
	        <table class="kqds_table">
			    		<tr>
			    			...
			    			<td style="text-align:right;"><font color="red">*</font>&nbsp;&nbsp;出库科室：</td>
			    			<td style="text-align:left;"> 
		    					 <select  name="deptpart" id="deptpart"></select>
			                </td>
			                <td style="text-align:right;"><font color="red">*</font>&nbsp;&nbsp;出库仓库：</td>
			                <td style="text-align:left;"> 
		    					 <select  name="sshouse" id="sshouse"></select>
			                </td>
			                <td style="text-align:right;"><font color="red">*</font>&nbsp;&nbsp;消耗原因：</td>
			    			<td style="text-align:left;">
			            ...
			            <div class="fixedHeaderDiv">
			            	 <table class="table-striped table-condensed table-bordered" style="width:100%;">
			            	    <thead style="height:30px; background: #00A6C0 ;color:white;">
									<tr>
										<th style="text-align: center; vertical-align: middle;width:10%"><span>操作</span></th>
										...
										<th style="text-align: center; vertical-align: middle;width:10%"><span>科室库存</span></th>
										<th style="text-align: center; vertical-align: middle;width:10%"><span>出库数量</span></th>
										<th style="text-align: center; vertical-align: middle;width:10%"><span>单位</span></th>
										<th style="padding-right:24px; text-align: center; vertical-align: middle;width:10%"><span>备注</span></th>
										<th style="text-align: center; vertical-align: middle;width:10%"><span>批号数量</span></th>
										<th style="text-align: center; vertical-align: middle;width:10%"><span>批号</span></th>
										<th style="text-align: center; vertical-align: middle;width:10%"><span>新增</span></th>
									</tr>
								</thead>
				            </table>
			            </div>
				            ...
				            <table id="table" class="table-striped table-condensed table-bordered" style="width:100%;" onkeydown="keyDown(event)">
			            	    <thead style="height:30px; background: #00A6C0 ;color:white;">
									<tr>
										...
										<th style="text-align: center; vertical-align: middle;width:10%"><span>出库数量</span></th>
										<th style="text-align: center; vertical-align: middle;width:10%"><span>单位</span></th>
										<th style="text-align: center; vertical-align: middle;width:10%"><span>备注</span></th>
										<th style="text-align: center; vertical-align: middle;width:10%"><span>批号数量</span></th>
										<th style="text-align: center; vertical-align: middle;width:10%"><span>批号</span></th>
										<th style="text-align: center; vertical-align: middle;width:10%"><span>新增</span></th>
									</tr>
...
$(function() {
  ...
  //更换仓库，置空商品列表
    $("#sshouse").change(function(){
    	...
    	$("#total1").html(0);
    	if($("#sshouse").val() == "8ef899f5-55c9-4940-a807-542bd6fe88c3") { //高值不允许内部消耗
    		$("#sshouse").val("");
    		layer.alert("高值耗材禁止内部消耗");
    	}
    });
  //更换科室，置空商品列表
...
function addDoodsDetail(detail){
     ...
     //商品主键8
 	 tablehtml += '<td style="display:none;"><span id="goodsid'+tdindex+'">' + detail.goodsid + '</span></td>';
 	 //批号数量9
     ...
     tablehtml += '<td style="width:auto;"><select type="text" style="width:100%; text-align:center;height: 20px;" onchange="selectAllPh(this,'+tdindex+')" name="ph'+tdindex+'" id="ph'+tdindex+'" class="ph"></select></td>';
 	 ...
 	 //下标12
     tablehtml += '<td style="width:10%;display:none;"><span id="index'+tdindex+'">'+tdindex+'</span></td>';
     tablehtml += "</tr>";
...
function deltr(obj) {
	...
	if(i<j){
		...
		$("#table").find("tbody").find("tr").each(function(i,obj){
			var goodsidAdd=$(this).find("td").eq(8).text();
...
function xzhw() {
	...
	layer.open({
        ...
        //点击遮罩关闭层
        area: ['90%', '450px'],
        content: contextPath + '/HUDH_KSllViewAct/toReplacementHouse.act?deptpart='+deptpart+'&sshouse='+sshouse
    });
}
function checknums(id,index){
	var kcnums = $("#kcnums"+index).html();//库存数量
	var nums = $("#nums"+index).val();//领用数量
	var goodsid = $("#goodsid"+index).html();//商品编号
	var phnum = $("#phnum"+index).html();//批号数量
	if(id == "nums"){
...
function selectAllPh(obj,tdindex) {
	...
	phObj.find(".phnum").html(phnum);
	var num=phObj.find("#nums"+tdindex).val();
	if(num){
