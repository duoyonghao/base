...
$(function (){
	if(!id){
		layer.alert("获取分类信息失败");
		var frameindex = parent.layer.getFrameIndex(window.name);
        parent.layer.close(frameindex); //再执行关闭
	}else {
		$.ajax({
			url: apppath + "/HUDH_YkzzTypeAct/findYkzzTypeById.act",
			type:"POST",
			dataType:"json",
			async:false,
			data:{"id":id},
			success:function(result){
				var ykzzType = result.ykzzType;
				if(ykzzType) {
					$("#perid").val(perId);
					$("#pername").val(perName);
					$("#goodstype").val(ykzzType.typeName);
					$("#sortno").val(ykzzType.orderno);
				}
			}
		});
	}
	...
	$("#tijiao").on("click",function (){
		...
		$.ajax({
			url: apppath + "/HUDH_YkzzTypeAct/updateYkzzTypeById.act",
			type:"POST",
			dataType:"json",
			async:false,
			data:{"id":id,
