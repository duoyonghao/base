    ...
    $("#table").bootstrapTable({
        ...
        columns: [
        {
		    ...
		    formatter: function(value, row, index) {
		    	var html = '';
			    html = '<span><a href="javascript:void(0);" mce_href="javascript:void(0);" style="color:green;" onclick="addExamineRemark(\'' + row.id + '\')">审核</a> </span>';
		        return html;
		    }
		},
		{
...
function delAll(id){
	...
	//询问框
	  layer.confirm('确定删除？', {
	    btn: ['删除','放弃'] //按钮
		  ...
		  var url = apppath + '/HUDH_YkzzDrugsInAct/deleteDrugsInById.act';
		    $.axse(url, paramOrder,
		    function(r) {
		        if (r.retState == "0") {
 ...
 //审核
 function addExamineRemark(drugsInId){
	 if(!adminName) {
		 layer.alert("请联系系统管理员配置药库审核人员");
		 return;
	 }
	 if(isAdmin == "false") {
		 ...
		 layer.open({
				...
				shadeClose: true, //点击遮罩关闭层
				area : ['40%' , '45%'],
				content: apppath+'/HUDH_YkzzViewAct/toAddExamineRemark.act?drugsInId=' + drugsInId
		});
	 }
 }
 ...
 function checkIdadmin (){
	 var url = apppath + '/HUDH_YkzzDrugsInAct/checkCurrentIsAdmin.act';
	 $.ajax({
			url: url,
			type:"POST",
			dataType:"json",
